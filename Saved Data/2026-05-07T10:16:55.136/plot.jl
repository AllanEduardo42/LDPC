...
modes_markers_lines =  ["UBP-RBP"              :cross           :solid
                        "C-RBP"                :none            :dot
                        "C&DR-RBP"             :circle          :solid
                        "C&DR-RBP 4"           :diamond         :solid
                        # "C&DR-RBP 5"           :utriangle       :solid
                 ]
directory = "./Saved Data/2026-05-07T10:16:55.136/"
liminf = -3.5
limsup = 0.1
...
for j=1:2
    for k in eachindex(EbN0)
        title = FB[j]*"ER $protocol (N = $N, R = $(R), Eb/N0 = $(EbN0[k])dB)"
        p = plot()
        for i in axes(modes_markers_lines,1)
            str = modes_markers_lines[i,1]
            x = readdlm(directory*FB[j]*"ER_"*str*".txt",'\t',Float64,'\n')
...
# display(p)
Plots.pdf(p,directory*"/FER_BER_$(N)")
