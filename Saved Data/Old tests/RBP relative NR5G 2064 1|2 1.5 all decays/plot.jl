...
plotlyjs()
directory = "./Saved Data/RBP relative $protocol $N $(R[1])|$(R[2]) $EbNo all decays/"
...
for j=1:2
    ...
    for i in eachindex(sdecays)
        x = readdlm(directory*FB[j]*"ER_RBP relative "*sdecays[i]*".txt",'\t',Float64,'\n')
        x = x[1:maxiter,:]
        labels = Vector{String}()
        push!(labels,"RBP relative "*sdecays[i])
