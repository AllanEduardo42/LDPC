...
modes_markers_lines_lines =  ["Flooding"             :none            :solid          
                        "LBP"                  :none            :solid
                        "RBP"                  :dtriangle       :solid
                        "RD-RBP 0.85"          :utriangle       :solid                
                        "NW-RBP"               :diamond         :solid         
                        "SVNF"                 :circle          :solid
                        "List-RBP (16,2) 0.85" :cross           :dot
                        "C&R-RBP 0.85"         :rect            :solid
                        "C&DR-RBP 0.85"        :xcross          :dot
                        #  "C-RBP 0.85"           :star5
                        #  "R-RBP 0.85"           :xcross
                 ]
...
FER_EbN0 = zeros(length(EbN0),size(modes_markers_lines_lines,1))
BER_EbN0 = zeros(length(EbN0),size(modes_markers_lines_lines,1))
...
for j=1:2
    for k in eachindex(EbN0)
        if active[k] == 1
            ...
            p = plot()
            for i in axes(modes_markers_lines_lines,1)
                str = modes_markers_lines_lines[i,1]
                ...
                p = plot!(
                    ...
                    x[1:maxiter],
                    ylabel=FB[j]*"ER",
                    label=modes_markers_lines_lines[i,1],
                    lw=3,
                    ls=modes_markers_lines_lines[i,3],
                    # title=title,
                    ylims=(liminf,limsup),
                    # xlim=(1,maxiter),
                    minorgrid=true,
                    ...
                    size = 1.5 .*(600,500),
                    markershape = modes_markers_lines_lines[i,2],
...
p = plot(p1,p2,layout=(2,1),bottom_margin=-1Plots.mm,)
display(p)
...
PLOT = plot(
    ...
    ylims=(10^(-5),1),
    color=colors',
    label=permutedims(modes_markers_lines_lines[:,1]),
    markershape=permutedims(modes_markers_lines_lines[:,2]),
    line=permutedims(modes_markers_lines_lines[:,3]),
    lw=2.5,
    markersize=5,
    markerstrokewidth = 0.25,
    guidefontsize=15,
