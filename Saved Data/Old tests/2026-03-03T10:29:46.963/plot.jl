...
modes =  ["RD-RBP", "List-RBP (16,2)", "C&R-RBP", "C&DR-RBP"]
                 
directory = "./Saved Data/2026-03-03T10:29:46.963/"

markers = [:diamond, :circle, :rect, :cross, :utriangle ]

marker_sizes = [4,4,4,5,4]

exponents = [-1.0,-1.0,-2.0,-6.0]
...
for ebn0 in eachindex(EbN0)
    ...
    for i in axes(modes,1)
        ...
        p2 = plot()
        if plot_mode == "gr"
            liminf = 1*10^exponents[ebn0]
            limsup = 1*1*10^(-0)
        else
            liminf = exponents[ebn0]
            limsup = -0
        end
        ...
        for k = 1:2        
            ...
            if plot_mode == "gr"
                liminf = 1*10^(exponents[ebn0]-1)
                limsup = 1*10^(-1)
            else
                liminf = exponents[ebn0]-1
                limsup = -1
            end
        end
        p = plot(p1,p2,layout=(2,1),bottom_margin=-3Plots.mm,)
        # display(p)
        Plots.pdf(p,directory*"/FER_BER_576_$(modes[i])_$ebn0")
    end
end
